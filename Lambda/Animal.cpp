...
auto getBMIlambdaOutsideClass([](double weight, double height)
{
...
// TODO : how to define lambda body outside H file ?
/*
std::function<double(void)> Animal::getBMIlambdaInCPPfile = [=]() -> double 
{ 
    if (height != 0)   // error handling for division by 0
    {
        return weight / (height * height);
...
const void Animal::describe() const
{
    std::cout << "Details of the animal are:"  << "\n"    << 
       ...
       "\t\t  Weight                                 : " << weight     << "\n"              <<
       "\t\t  Height                                 : " << height     << "\n"              <<
       "\t\t  BMI : member func (normal)             : " << getBMI()   << " ("              << get_weight_type() << ")\n" <<
       "\t\t  BMI : non-member func (lambda)         : " << getBMIlambdaOutsideClass(weight, height)   << "\n"                       <<
       "\t\t  BMI : member func (lambda in H file)   : " << getBMIlambdaInHfile()  << "\n"        <<
       "\t\t  BMI : member func (lambda in CPP file) : " << "TODO\n"       <<  
       "\t\t  BMI : direct (lambda)                  : " << "TODO\n"       <<  
